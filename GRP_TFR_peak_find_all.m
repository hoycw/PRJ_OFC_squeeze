...
nobl_id = 'TFRw_S25t2_noBsln_fl2t40_c7';%'TFRw_S25t2_noBsln_fl2t40_c7';
...
for s = 1:length(SBJs)
    ...
    nobl_fname = [sbj_dir SBJs{s} '_' nobl_id '.mat'];
    fprintf('Loading %s\n',proc_fname);
    tmp = load(proc_fname,'tfr');
    tmp2 = load(nobl_fname,'tfr');
    ...
    if s==1
        ...
        psd_nobl = nan([length(SBJs) 2 numel(tmp.tfr.freq)]);
    end
    ...
    psd_tfr_nobl = ft_selectdata(cfgs,tmp2.tfr);
    psd(s,:,:) = squeeze(nanmean(nanmean(psd_tfr.powspctrm,1),4));
    psd_nobl(s,:,:) = squeeze(nanmean(nanmean(psd_tfr_nobl.powspctrm,1),4));
    
    % Find PSD peaks
    for ch_ix = 1:2
        % no baseline
        [pks,pk_ix] = findpeaks(squeeze(psd_nobl(s,ch_ix,:)));
        peaks{s,ch_ix,1} = freq_vec(pk_ix);
        peak_sz{s,ch_ix,1} = pks;
        % baseline corrected
        [pks,pk_ix] = findpeaks(squeeze(psd(s,ch_ix,:)));
        peaks{s,ch_ix,2} = freq_vec(pk_ix);
        peak_sz{s,ch_ix,2} = pks;
    end
end

%% Normalize peak size
peak_szn = cell(size(peak_sz));
all_peaks_nobl = [vertcat(peak_sz{:,1,1}); vertcat(peak_sz{:,2,1})];
all_peaks_bc = [vertcat(peak_sz{:,1,2}); vertcat(peak_sz{:,2,2})];
for s = 1:numel(SBJs)
    for ch_ix = 1:2
        peak_szn{s,ch_ix,1} = (peak_sz{s,ch_ix,1}-mean(all_peaks_nobl))./std(all_peaks_nobl);
    end
end
...
for s = 1:numel(SBJs)
    for ch_ix = 1:2
        subplot(2,4,s+4*(ch_ix-1)); hold on;
        scatter(peaks{s,ch_ix,1},ones(size(peaks{s,ch_ix,1})),zscore(peak_sz{s,ch_ix,1})*scat_scale+scat_sz,'b');
        scatter(peaks{s,ch_ix,2},2*ones(size(peaks{s,ch_ix,2})),zscore(peak_sz{s,ch_ix,2})*scat_scale+scat_sz,'r');
        ylim([0.5 2.5]);
        xlim([1 40]);
        set(gca,'YTick',[1 2]);
        set(gca,'YTickLabels',{'no bsln','bsln corrected'});
        xlabel('Peaks in Frequency (Hz)');
        if ch_ix==1
            title([SBJs{s} ': ' sbj_bg_roi{s}]);
        else
            title([SBJs{s} ': ' sbj_pfc_roi{s}]);
        end
        set(gca,'FontSize',16);
    end
end
