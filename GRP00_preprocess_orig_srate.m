...
% *_orig_srate for PNAS Revision 1: This version omits the resampling to 1 kHz to verify
%   that doesn't affect the results
%   Also excludes plotting of behavioral data
clear all
close all
clc
...
%     close all;
    ...
    for r_ix = 1:2
        ...
        % Synchronise data- CWH: This version does NOT resample to 1 kHz
        % Set the resample rate of PCS data to be the same as the sampling rate in the Matlab /
        % Biopacs
        srate = SyncDetails(s,7);%422 Hz for PC+S, 500 Hz for RC+S (PFC01)
        ...
        %% Fill NaNs
        for ch_ix = 1:2
            nrl_fill(ch_ix,:) = fillmissing(signal(ch_ix,:),'linear');
        end
        
        %% Plot PSDs of the data
        if plot_it
            [pxx1,f] = pwelch(nrl_fill(1,:),srate,0,[1:100],srate);
            [pxx2,f] = pwelch(nrl_fill(2,:),srate,0,[1:100],srate);

            fig_name = [SBJs{s} '_run_' num2str(r_ix) '_PSDs_osr'];
            ...
            fig_fname = [fig_dir 'PSDs_osr/' fig_name '.' fig_ftype];
            fprintf('Saving %s\n',fig_fname);
        ...
        % PCS_start is sample number when srate = 1 kHz (I think...)
        %   PCS_start_osr converts to time, then back to sample number using native sample rate
        PCS_start_osr = round((PCS_start/new_srate)*srate);
        nrl_trim = nrl_fill(:,PCS_start_osr:end);
...
%         run_data{r_ix}.sampleinfo = nan([n_choice_trl 2]);
        for trl_ix = choice_trl_ix
            ...
            trl_lim_samp = trl_lim.*srate;
            if strcmp(evnt_lab,'full')
                start_samp = round(trial_onset*srate) + trl_lim_samp(1);
                end_samp   = round(choice_onset*srate) + trl_lim_samp(2);
            else
                if strcmp(evnt_lab,'stim')
                    event_samp = round(trial_onset*srate);
                elseif strcmp(evnt_lab,'dec')
                    event_samp = round(choice_onset*srate);
                else
            ...
            if all(~isnan([start_samp end_samp])) % skip choice trials with no response
                ...
                run_data{r_ix}.time{trl_ix} = ([start_samp:end_samp]-start_samp+trl_lim_samp(1))./srate;
...
%                 run_data.trialinfo(trl_ix,2)=trls1(trl_ix).stakeIx;
        ...
        clear syncR1 syncR2 result signal nrl_trim nrl_fill
%         close all
    end
...
%         trialmean(trl_ix,:) = mean(abs(tmp),2);    % why are you taking abs here?
    ...
    % fprintf('var_rej: ');fprintf('%d,',bhv.trl(var_reject_ix));fprintf('\n');
    % fprintf('man_rej: ');fprintf('%d,',bhv.trl(man_trl_rej_ix{s}));fprintf('\n');
    
    %% Reject neural outliers
    % Rejection based on criteria listed in parameter definition
    % THIS VERSION USES VAR_REJECT_IX instead of man_trl_rej_ix
    if isempty(var_reject_ix)
        fprintf('%s: No bad trials identified.\n',SBJs{s});
    else
        fprintf(2,'%s: Removing %d trials!\n',SBJs{s},length(var_reject_ix));
    end
    
    % Remove outlier trials
    data.trial(var_reject_ix) = [];
    data.time(var_reject_ix)  = [];
%     data.trialbl(bad_trl_ix) = [];
%     data.sampleinfo(bad_trl_ix,:) = [];
    
    bhv.reject_ix = var_reject_ix;
    bhv_fields = fieldnames(bhv);
    n_good_bhv = length(bhv.trl);
    for f = 1:length(bhv_fields)
        if length(bhv.(bhv_fields{f}))==n_good_bhv && ~contains(bhv_fields{f},'_ix')
            bhv.(bhv_fields{f})(var_reject_ix) = [];
        end
    end
...
%     sbj_data.TFbl    = freqoutbl;
    ...
    out_fname = [out_dir preproc_name '_preproc_osr.mat'];
    fprintf('Saving %s\n',out_fname);
