            ...
            for svre_ix = 1:3
                ...
                if strcmp(comb_method,'mn')
                    ...
                    theta_sem(s,ch_ix,svre_ix,split_ix,:) = ...
                        squeeze(std(theta_pow.powspctrm(idx,ch_ix,1,:),[],1))./sqrt(sum(idx));
                    beta_avg(s,ch_ix,svre_ix,split_ix,:) = squeeze(mean(beta_pow.powspctrm(idx,ch_ix,1,:),1));
                    beta_sem(s,ch_ix,svre_ix,split_ix,:) = ...
                        squeeze(std(beta_pow.powspctrm(idx,ch_ix,1,:),[],1))./sqrt(sum(idx));
                elseif strcmp(comb_method,'md')
                    ...
                    theta_sem(s,ch_ix,svre_ix,split_ix,:) = squeeze(mad(theta_pow.powspctrm(idx,ch_ix,1,:),1,1));
                    beta_avg(s,ch_ix,svre_ix,split_ix,:) = squeeze(median(beta_pow.powspctrm(idx,ch_ix,1,:),1));
                    beta_sem(s,ch_ix,svre_ix,split_ix,:) = squeeze(mad(beta_pow.powspctrm(idx,ch_ix,1,:),1,1));
                else
