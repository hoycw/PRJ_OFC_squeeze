...
pfc_ch_ix = find(strcmp(DataStr(1).data.TFbl.label,'OFC'));
...
theta_pfc   = nan(length(SBJs),length(tmes));
beta_pfc    = nan(length(SBJs),length(tmes));
betapk_pfc  = nan(length(SBJs),length(tmes));
pwr_pfc     = nan(length(SBJs),length(frqs),length(tmes));
...
for s=1:length(SBJs)
    ...
    if ~strcmp(DataStr(s).data.TFbl.label{pfc_ch_ix},'OFC'); error('PFC channel index mismatch'); end
    ...
    theta_pfc(s,:)  = squeeze(mean(pwr_mean(pfc_ch_ix,th_ix(1):th_ix(2),:),2));
    beta_pfc(s,:)   = squeeze(mean(pwr_mean(pfc_ch_ix,b_ix(1):b_ix(2),:),2));
    betapk_pfc(s,:) = squeeze(mean(pwr_mean(pfc_ch_ix,bpk_ix(s,1):bpk_ix(s,2),:),2));
    pwr_pfc(s,:,:)  = squeeze(pwr_mean(pfc_ch_ix,:,:)); % [sbj, freq, time]
...
%% Plot Group-level PFC and LFP power
% find time indices
...
theta_pfc_sem = std(theta_pfc,[],1)./sqrt(length(SBJs));
theta_lfp_sem = std(theta_lfp,[],1)./sqrt(length(SBJs));
beta_pfc_sem  = std(beta_pfc,[],1)./sqrt(length(SBJs));
...
% Plot PFC theta
subplot(2,3,1);
shadedErrorBar(plot_tmes, mean(theta_pfc(:,plt_tme_idx),1),theta_pfc_sem(:,plt_tme_idx));%, 'transparent',sem_alpha);
line([0 0],ylim,'Color','k','LineStyle','-');
xlabel('Time (s)','Fontsize',10,'Fontname','Arial');
ylabel('PFC Theta Power');
title(['Group averaged (n=' num2str(length(SBJs)) ') PFC theta power']);
% xlim([-0.5 2]);
set(gca,'FontSize',16);

% Plot PFC beta
subplot(2,3,2); hold on;
b_line   = shadedErrorBar(plot_tmes, mean(beta_pfc(:,plt_tme_idx),1),...
    beta_pfc_sem(:,plt_tme_idx),'lineprops',{'Color','r'});%, 'transparent',sem_alpha);
bpk_line = shadedErrorBar(plot_tmes, mean(betapk_pfc(:,plt_tme_idx),1),...
    beta_pfc_sem(:,plt_tme_idx),'lineprops',{'Color','b'});%, 'transparent',sem_alpha);
line([0 0],ylim,'Color','k','LineStyle','-');

xlabel('Time (s)','Fontsize',10,'Fontname','Arial');
ylabel('PFC Beta Power');
title(['Group averaged (n=' num2str(length(SBJs)) ') PFC beta power']);
% xlim([-0.5 2]);
set(gca,'FontSize',16);
...
subplot(2,3,3);
% clims=[0 3.5];
imagesc(plot_tmes, frqs, squeeze(mean(pwr_pfc(:,:,plt_tme_idx),1)));%,clims);
line([0 0],ylim,'Color','k','LineStyle','-');
...
ylabel('Frequency (Hz)');
colorbar;
title(['Group averaged (n=' num2str(length(SBJs)) ') PFC TFR']);
set(gca,'FontSize',16);
...
for ch_ix = 1:length(ch_lab)
    ...
    ylabel('Trials');
    colorbar;
    title([SBJs{s} ': ' ch_str ' single-trial theta']);
    set(gca,'FontSize',16);
    ...
    ylabel('Trials');
    colorbar;
    title([SBJs{s} ': ' ch_str ' single-trial beta']);
    set(gca,'FontSize',16);
end
