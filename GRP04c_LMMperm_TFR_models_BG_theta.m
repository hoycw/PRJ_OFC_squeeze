...
fit_method = 'ML';
full_formula = 'BG_theta~ reward_cur + effortS_cur + reward_prv + effortS_prv + (1|sbj_n)';% + (1|trl_n_cur)');
predictors = {'reward_cur','effortS_cur','reward_prv','effortS_prv'};
[lme_full,pred_pval] = fn_run_LMM_full_permutation_models(good_tbl_prv.BG_theta,predictors,full_formula,fit_method,st.nboots);

%% Plot BG theta ~ previous reward as line plot
fit_method = 'ML';
full_formula = 'BG_theta~ reward_cur + effortS_cur + reward_prv + effortS_prv + (1|sbj_n)';
lme_full     = fitlme(good_tbl_prv.BG_theta,full_formula,'FitMethod',fit_method);
noRp_formula = 'BG_theta~ reward_cur + effortS_cur + effortS_prv + (1|sbj_n)';
lme_noRp     = fitlme(good_tbl_prv.BG_theta,noRp_formula,'FitMethod',fit_method);
...
sv_formula = 'BG_theta~ SV_cur + SV_prv + (1|sbj_n)';
sv_pred = {'SV_cur','SV_prv'};
[lme_sv,sv_pred_pval] = fn_run_LMM_full_permutation_models(good_tbl_prv.BG_theta,sv_pred,sv_formula,fit_method,st.nboots);
bg_theta_full_vs_SV = compare(lme_sv,lme_all,'NSim',1000)
